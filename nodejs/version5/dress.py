...
def testFunc(upwidth,neck,neckx,bottom):
    upwidth,neck,neckx,bottom=int(upwidth),int(neck),int(neckx),int(bottom)
    f = open("./dress.txt","r")
    read_data=f.read()
    f.close()
    ...
    gap=upwidth/Width
    Width=upwidth
    Height=int(Height*gap)+20
    if Height > bottom-neck-10:
        Hieght=bottom-neck-10
